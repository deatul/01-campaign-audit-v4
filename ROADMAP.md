...
- [x] Derive the expected canonical company set from independently collected
      source records, not from `plan.source_row_ids`.
- [x] Compare exact sets and report missing, unexpected, and duplicated companies
      separately.
- [x] Compare deliverables as a multiset keyed by `(company_id, asset_type)` so
      duplicate assets cannot disappear inside a Python `set`.
- [x] Require exactly one of every required asset type per company.
- [x] Validate source-row traceability, company identity, brand kit, template,
      and the completion evidence.
- [x] Verify that every source row is represented either by a valid canonical
      company/provenance entry or by exactly one skipped-row diagnostic, including
      a distinct duplicate classification.
- [x] Return structured diagnostics suitable for the demo and audit reports.

### 7. Add regression and property-oriented tests

- [x] Add tests for every loader in `src/sources.py`, classifying each as safely
      recoverable or necessarily rejected.
- [x] Assert that normal pagination yields the same canonical companies and
      deliverables for page sizes 1, 10, 25, 100, and sizes larger than the list.
- [x] Assert that exact page replay does not duplicate source rows, companies, or
      deliverables.
- [x] Assert prompt failure for stalled cursors, cycles, and missing cursors.
- [x] Assert refusal for silent truncation and unstable reordering unless the
      enhanced loader contract supplies trustworthy completeness evidence.
- [x] Test duplicate companies split across page boundaries, positional
...
- [x] Mutate a valid plan by removing or duplicating companies/assets and by
      changing brand/template IDs; every mutation must fail coverage.
- [x] Run the same checks against `fixtures/second_list.json` to demonstrate that
      the solution generalizes beyond the reported fixture.

### 8. Update reporting and operational behavior

- [x] Keep `make demo`, `make test`, `make verify`, and `make audit` working.
- [x] Make demo/audit output distinguish `complete`, `rejected`, and `failed`
      rather than presenting every returned dictionary as a completed campaign.
- [x] Report counts for source rows, canonical companies, duplicate rows merged,
      rejected identities, and generated deliverables.
- [x] Document which historical paging shapes are handled and which are refused,
